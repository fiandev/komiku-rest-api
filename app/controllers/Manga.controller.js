...
const { mangaHomeUrl, baseUrl2 } = require('../constants/url')
...
class MangaController extends Controller {
    async all() {
        ...
        try {
            ...
            const url = (pageNumber === '1')
                ? `${baseUrl2}/pustaka/?orderby=modified&genre&genre2&status&category_name=manga`
                : `${baseUrl2}/pustaka/page/${pageNumber}/?orderby=modified&genre&genre2&status&category_name=manga`
    ...
    async popular() {
        try {
            const { data } = await axios.get(mangaHomeUrl)
            const selector = cheerio.load(data)
