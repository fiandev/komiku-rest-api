...
const { manhwaHomeUrl, baseUrl2 } = require('constants/url')
...
class ManhwaController extends Controller {
    async all() {
        ...
        try {
            ...
            const url = (pageNumber === '1')
                ? `${baseUrl2}/pustaka/?orderby=modified&genre&genre2&status&category_name=manhwa`
                : `${baseUrl2}/pustaka/page/${pageNumber}/?orderby=modified&genre&genre2&status&category_name=manhwa`
    ...
    async popular() {
        try {
            const { data } = await axios.get(manhwaHomeUrl)
            const selector = cheerio.load(data)
